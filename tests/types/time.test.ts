...
test('integers', ()=>{
    for(let mul = 1; mul < 1e10; mul*=10){
        for(let num = 0; num < 10; num++){
            let input = num * mul;
            let result = c.Time.decode(input);
...
test('strings-seconds-only', ()=>{
    for(let input = 0; input < 60; input++){
        let result = c.Time.decode(input.toString());
        expect(result._tag).toBe('Right');
        if(result._tag == 'Right')
            expect(result.right).toBe(input*1000);
    }
});
...
test('strings-minute-seconds', ()=>{
    for(let input = 0; input < 60; input++){
        let result = c.Time.decode(`${59-input}:${input}`);
        expect(result._tag).toBe('Right');
        if(result._tag == 'Right')
...
test('strings-mix', ()=>{
    for(let input = 1, sign=1; input < 1e10; input*=Math.E, sign*=-1){
        let result = c.Time.decode(Format(sign*input));
        expect(result._tag).toBe('Right');
        if(result._tag == 'Right')
