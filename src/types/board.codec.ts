...
function filter<L, R>(a: Either<L, R>[]): {left: L[], right: R[]} {
    return a.reduce((result: {left:L[], right:R[]}, c)=>{
...
export const Board = new io.Type<BoardType, string[][], unknown>(
    ...
    (i: unknown, context: io.Context) => { 
        if(Array.isArray(i)){
            let board: Either<io.Errors, ColorType[]>[] = _.map(i, (r, index)=>{
                if(Array.isArray(r)){
                    if(r.length === i.length){
                        let row: Either<io.Errors, ColorType>[] = _.map(r, (c)=>ColorCodec.decode(c));
                        let result = filter(row);
                        if(result.left.length > 0) return io.failure(i, context, result.left.map(error=>error.toString).join(','));
                        return io.success(result.right);
                    } else return io.failure(i, context, `Row#${index} does not have the same length as the columns`)
                } else return io.failure(i, context, `Row#${index} is not an array`);
            });
            let result = filter(board);
            if(result.left.length > 0) return io.failure(i, context, result.left.map(error=>error.toString).join(','));
            return io.success(result.right);
