...
export const GameState = new io.Type<t.GameState, any, unknown>(
    ...
    (u: any): u is t.GameState => {
        let g = u as t.GameState;
        return  g.board && BoardCodec.is(g.board) &&
                g.players &&
                ColorType.BLACK in g.players && PlayerStateCodec.is(g.players[ColorType.BLACK]) &&
                ColorType.WHITE in g.players && PlayerStateCodec.is(g.players[ColorType.WHITE]) &&
                g.turn && ColorCodec.is(g.turn);
    },
    (i: unknown, c: io.Context) => {
        if(i && typeof i === 'object' && i !== null) {
            let obj: {board:unknown, players:unknown, turn:unknown} = i as {board:unknown, players:unknown, turn:unknown};
            let turn: ColorType;
            if(obj.turn){
                let result = ColorCodec.decode(obj.turn);
            ...
            if(obj.players){
                if(typeof obj.players === 'object' && obj.players !== null){
                    let p: {[index: string]:unknown} = obj.players as {[index: string]:unknown};
                    if(ColorType.BLACK in p && ColorType.WHITE in p){
                        let black = PlayerStateCodec.decode(p[ColorType.BLACK]);
                        let white = PlayerStateCodec.decode(p[ColorType.WHITE]);
                        if(black._tag === "Left") return black;
                        if(white._tag === "Left") return white;
                        players = {[ColorType.BLACK]: black.right, [ColorType.WHITE]:white.right};
                    } else {
                        let result = PlayerStateCodec.decode(p);
                        if(result._tag == "Left") return result;
                        else players = {[ColorType.BLACK]: result.right, [ColorType.WHITE]:result.right};
                    }
            ...
            if(obj.board){
                if(Array.isArray(obj.board)){
                    let result = BoardCodec.decode(obj.board);
                    if(result._tag === "Left") return result;
                    ...
                    } else return io.failure(i, c, `Board size must be an integer, value received is ${obj.board}`);
                } else {
                    return io.failure(i, c, `Board must be either a 2D array or Integer, value received is ${obj.board}`);
                }
            } else {
