...
export const GameConfiguration = io.interface(
  {
    ...
    ko: ioe.withDefault(io.boolean, false),
    superko: ioe.withDefault(io.boolean, true),
    mercy: ioe.withDefault(io.number, 0),
    mercyStart: ioe.withDefault(io.number, 100),
    scoringMethod: ioe.withDefault(
      io.union([io.literal('area'), io.literal('territory')]),
      'area'
    ),
    prisonerScore: ioe.withDefault(io.number, 1),
    passAddsToPrisoners: ioe.withDefault(io.boolean, false)
  },
  'GameConfiguration'
);
