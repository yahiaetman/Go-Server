...
socket.on('message', data => {
  try {
    ...
    ); // Parse the message and try to decode it
    if (result._tag == 'Left') {
      // If the message is undecodable, ignore it
...
rl.on('line', input => {
  ...
  const args = input
    .toLowerCase()
    .trim()
    .split(/\s+/, 2); // Clean and split the input
  switch (args[0] ?? '') {
    case 'p':
    case 'pass': // If the command is pass, then.... the move is a pass I believe
      move = { type: 'pass' };
      break;
    case 'r':
    case 'resign': // Also, If the command is resign, then.... the move is a resign I think
      move = { type: 'resign' };
      break;
    case 'place': {
      // This is a place move
      if (args.length < 2) {
        // The must be a point with the `place` command
        console.error('Please enter a point');
        rl.prompt();
        return;
      }
      const point = PointUtility.parse(args[1]); // Parse the point
      if (point == null) {
    ...
    default: {
      const point = PointUtility.parse(args[0]); // If it is neither of the above, the user may have entered a point only meaning to do a place, so we will try to parse it
      if (point == null) {
...
function processEvent(event: ClientEvent) {
  if (event.type == 'DISCONNECTION') {
    // The client was disconnected, so we bail out.
    console.log(`Disconnetion: ${event.code} - ${event.reason}`);
    process.exit(0);
  }
  if (event.type == 'MESSAGE' && event.message.type == 'END') {
    // The game has ended so we display, the end-game info
  ...
  if (state == ClientState.INIT) {
    // While the client is initializing
    ...
    // While the client is ready to play a game
    if (event.type == 'MESSAGE') {
      // If this is a server message
      switch (event.message.type) {
        case 'START': // And the message type is NAME
          ...
          // If it is my turn, go to THINKING
          else state = ClientState.IDLE; // otherwise, go to IDLE
          break;
      }
    }
  } else if (state == ClientState.IDLE) {
    // While the client is wating for the opponent's move
    if (event.type == 'MESSAGE') {
      // If this is a server message
      switch (event.message.type) {
        case 'MOVE': // And the message type is MOVE
          const deltaTime =
            ...
            ) - _.sum(_.map(event.message.remainingTime)); // Get the difference in time
          game.apply(event.message.move, deltaTime); // Update the game state
          switch (
            event.message.move.type // Print a message about the opponent's move
          ) {
            case 'pass':
    ...
    // While the client is thinking of a move
    if (event.type == 'INPUT') {
      // If this is a user input
    ...
    // While the client is awaiting the server response
    if (event.type == 'MESSAGE') {
      // If this is a server message
      switch (event.message.type) {
