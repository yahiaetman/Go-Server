...
const gameManager = new GameManager({
  tick: () => {},
    ...
    console.log('\nGame Over');
    const endInfo = gameManager.EndGameInfo;
    switch (endInfo?.reason) {
      case 'pass':
        console.log('Reason: Subsequent Pass Moves');
        break;
      case 'resign':
        console.log('Reason: Resignation');
        break;
      case 'timeout':
    ...
    console.log(
      `Winner: ${endInfo?.winner == Color.BLACK ? 'Black' : 'White'}`
    );
...
rl.on('line', (input: string) => {
  ...
  switch (args[0] ?? '') {
    ...
    case 'place':
      ...
      const point = PointUtility.parse(args[1]);
      if (point == null) {
  ...
  const result = gameManager.apply(move);
  console.log(gameManager.toString(true));
  rl.prompt();
}).on('close', () => {
